...
def update_plot(frame):
    ...
    # Get data for JOY_C and eq_joy_


    data1 = np.random.randint(1, 101, 10)
    data2 = np.random.randint(1, 101, 10)

    x_data = list(range(len(data1)))
    joy_c_plot.set_data(x_data, data1)
    eq_joy_c_plot.set_data(x_data, data2)
    ax[0].relim()
    ax[0].autoscale_view()

    # Get data for JOY_U and eq_joy_u
    x_data = list(range(len(data2)))
    joy_u_plot.set_data(x_data, data2)
    eq_joy_u_plot.set_data(x_data, data1)
    ax[1].relim()
    ax[1].autoscale_view()
